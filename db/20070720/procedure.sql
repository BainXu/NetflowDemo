	...
	BEGIN
		...
		BEGIN
			...
			BEGIN
				...
				SELECT count(*) INTO counter2_ FROM NFWeekTab WHERE engineId = engineId_ AND targetId = targetId_ AND targetName = targetName_ AND week = time_;
				IF counter2_ > 0 THEN
                	UPDATE NFWeekTab SET counts = counts_, pkts = pkts_, bytes = bytes_
                    	WHERE engineId = engineId_ AND targetId = targetId_ AND targetName = targetName_ AND week = time_;
				ELSE
				...
				SELECT count(*) INTO counter3_ FROM NFMonthTab WHERE engineId = engineId_ AND targetId = targetId_ AND targetName = targetName_ AND month = time_;
				IF counter3_ > 0 THEN
					UPDATE NFMonthTab SET counts = counts_, pkts = pkts_, bytes = bytes_
						WHERE engineId = engineId_ AND targetId = targetId_ AND targetName = targetName_ AND month = time_;
				ELSE
